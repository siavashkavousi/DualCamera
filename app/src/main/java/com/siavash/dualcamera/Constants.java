package com.siavash.dualcamera;

/**
 * App Constants
 * Created by sia on 8/18/15.
 */
public class Constants {
    //region Camera Fragments
    public static final String CAMERA_FRONT_IMAGE_URL = "camera_front_url";
    public static final String CAMERA_BACK_IMAGE_URL = "camera_back_url";
    public static final String CAMERA_BACK = "camera_back";
    public static final String CAMERA_FRONT = "camera_front";
    //endregion

    //region Camera Preview
    public static final int DISPLAY_ORIENTATION = 90;
    public static final int COMPRESS_QUALITY = 100;
    //endregion

    public static final int CONTAINER_RES_ID = R.id.container;
    public static final String PHOTO_FRAGMENT = "photoFragment";
    public static final String BACK_CAMERA_FRAGMENT = "backCameraFragment";
    public static final String FRONT_CAMERA_FRAGMENT = "frontCameraFragment";
}
